...
from .models import Skin, Game, Cart, CartItem, Order, OrderItem, Review, UserProfile
from .forms import SkinForm, UserRegisterForm, ReviewForm, ProfileForm
...
def skin_detail(request, pk):
    ...
    similar_skins = Skin.objects.filter(game=skin.game, status='available').exclude(pk=pk)[:4]
...
def sell_skin(request):
    # Prefill from Steam inventory item via GET params
    initial = {}
    if request.GET.get('name'):
        initial['name'] = request.GET.get('name')
    if request.GET.get('image_url'):
        initial['image_url'] = request.GET.get('image_url')
...
def cart(request):
    ...
    return render(request, 'skins/cart.html', {'cart': cart, 'items': items})
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        order = Order.objects.create(buyer=request.user, total_price=total, status='paid')
        for item in items:
...
def game_skins(request, slug):
    ...
    return render(request, 'skins/game_skins.html', {
        'game': game, 'skins': skins, 'games': games,
    })


@login_required
def steam_inventory(request):
    from .steam import get_steam_inventory
    prof, _ = UserProfile.objects.get_or_create(user=request.user)
    inventory = {}
    error = None

    if request.method == 'POST' and request.POST.get('steam_id'):
        prof.steam_id = request.POST.get('steam_id')
        prof.save()
        messages.success(request, 'Steam ID збережено!')
        return redirect('steam_inventory')

    if prof.steam_id:
        try:
            inventory = get_steam_inventory(prof.steam_id)
        except Exception as e:
            error = str(e)

    return render(request, 'skins/steam_inventory.html', {
        'inventory': inventory,
        'has_steam_id': bool(prof.steam_id),
        'steam_id': prof.steam_id,
        'error': error,
        'profile': prof,
    })
